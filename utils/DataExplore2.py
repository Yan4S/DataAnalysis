...
class DataPlotter:
    ...
    def _setup_styles(self):
        """Setup matplotlib styles for better plots"""
    ...
    def _setup_numeric_controls(self, X: pd.DataFrame):
        ...
        # Display controls in an organized layout
        display(HTML("<h3>Numeric Features Analysis</h3>"))
        controls_box = widgets.HBox([plot_type, columns_per_row, update_btn])
        controls_box.layout.justify_content = 'flex-start'
        controls_box.layout.align_items = 'flex-end'
    ...
    def _plot_numeric_features(self, X: pd.DataFrame, plot_type: str, ncols: int):
        ...
        if not numeric_cols:
            display(HTML("<p>No numeric columns to plot</p>"))
            return
        ...
        for i, col in enumerate(numeric_cols):
            if i < len(axes_flat):
                ...
                try:
                    ...
                    ax.set_title(f'{col} (Error)', fontsize=10)
    ...
    def _setup_categorical_controls(self, X: pd.DataFrame):
        ...
        controls_box = widgets.HBox([max_categories, columns_per_row, update_btn])
        controls_box.layout.justify_content = 'flex-start'
        controls_box.layout.align_items = 'flex-end'
        display(controls_box)
        display(output)
    ...
    def _plot_categorical_features(self, X: pd.DataFrame, max_cats: int, ncols: int):
        ...
        if not categorical_cols:
            display(HTML("<p>No categorical columns to plot</p>"))
            return
    ...
    def plot_missing_values(self, X: pd.DataFrame):
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, max(6, len(missing_data) * 0.3)))
        ...
        for bar in bars1:
            width = bar.get_width()
            if width > 0:  # Only add text if there's space
                ax1.text(width, bar.get_y() + bar.get_height()/2., f'{int(width)}', 
                        ha='left', va='center', fontsize=9)
        
        for bar in bars2:
            width = bar.get_width()
            if width > 5:  # Only add text if there's space
    ...
    def _setup_correlation_controls(self, X: pd.DataFrame):
        ...
        controls_box = widgets.HBox([n_features, correlation_type, threshold, update_btn])
        controls_box.layout.justify_content = 'flex-start'
        controls_box.layout.align_items = 'flex-end'
        display(controls_box)
        display(output)
    ...
    def _plot_correlation_heatmap(self, X: pd.DataFrame, n_features: int, method: str, threshold: float):
        ...
        try:
            ...
            display(HTML(f"<p style='color: red;'>Error generating correlation matrix: {str(e)}</p>"))
    ...
    def _setup_target_controls(self, X: pd.DataFrame, y: pd.Series):
        ...
        controls_box = widgets.HBox([n_features, plot_type, update_btn])
        controls_box.layout.justify_content = 'flex-start'
        controls_box.layout.align_items = 'flex-end'
        display(controls_box)
        display(output)
    ...
    def _plot_target_relationships(self, X: pd.DataFrame, y: pd.Series, n_features: int, plot_type: str):
        ...
        try:
            ...
            display(HTML(f"<p style='color: red;'>Error in target analysis: {str(e)}</p>"))
    ...
    def _show_high_correlations(self, corr_matrix: pd.DataFrame, threshold: float):
        ...
        try:
            ...
            display(HTML(f"<p style='color: red;'>Error analyzing high correlations: {str(e)}</p>"))
